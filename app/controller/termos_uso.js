...
router.get('/', function(req, res, next) {
	...
	data[req.session.usuario.id+'_numero_menu'] = 10;
	...
	usuarioStatusModel.find({id_usuario:mongoose.Types.ObjectId(req.session.usuario.id)},function(err,data_usuario_status){
		...
		res.render(req.isAjaxRequest() == true ? 'api' : 'montador', {html: 'suporte/termos_uso', data: data, usuario: req.session.usuario});
	}).sort({'_id':-1}).limit(1);
});
